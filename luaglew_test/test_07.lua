...
function cnv:action(x, y)
  ...
  luaglew.glBindTexture(luaglew.GL_TEXTURE_2D, texture[filter])
...
function cnv:k_any(c)
  ...
  if c == iup.K_l then   -- L Key Being Pressed ?
    if (light) then
      luaglew.glDisable(luaglew.GL_LIGHTING)
      light = false
    else
      luaglew.glEnable(luaglew.GL_LIGHTING)
      light = true
    end
  end
...
function LoadGLTextures()
  ...
  luaglew.glTexParameterf(luaglew.GL_TEXTURE_2D,luaglew.GL_TEXTURE_MIN_FILTER,luaglew.GL_NEAREST)
  luaglew.glTexParameterf(luaglew.GL_TEXTURE_2D,luaglew.GL_TEXTURE_MAG_FILTER,luaglew.GL_NEAREST)

  crate = LoadTGA('crate.tga')

  luaglew.glTexImage2D(luaglew.GL_TEXTURE_2D, 0, 4, crate.width, crate.height, 0, luaglew.GL_RGBA, luaglew.GL_UNSIGNED_BYTE, crate)

  -- Create Linear Filtered Texture
  luaglew.glBindTexture(luaglew.GL_TEXTURE_2D, texture[2])
  ...
  luaglew.glBindTexture(luaglew.GL_TEXTURE_2D, texture[3])
  luaglew.glTexParameterf(luaglew.GL_TEXTURE_2D,luaglew.GL_TEXTURE_MIN_FILTER,luaglew.GL_LINEAR_MIPMAP_NEAREST)
  luaglew.glTexParameterf(luaglew.GL_TEXTURE_2D,luaglew.GL_TEXTURE_MAG_FILTER,luaglew.GL_LINEAR)
...
function cnv:map_cb()
  ...
  luaglew.glLightfv(luaglew.GL_LIGHT1, luaglew.GL_AMBIENT, LightAmbient)        -- Setup The Ambient Light
  luaglew.glLightfv(luaglew.GL_LIGHT1, luaglew.GL_DIFFUSE, LightDiffuse)        -- Setup The Diffuse Light
  luaglew.glLightfv(luaglew.GL_LIGHT1, luaglew.GL_POSITION, LightPosition)      -- Position The Light

  luaglew.glEnable(luaglew.GL_LIGHT1)


end

dlg = iup.dialog{cnv; title="luaglew test 7"}
